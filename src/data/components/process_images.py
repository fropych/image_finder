...
def main(cfg: DictConfig):
    ...
    template_paths = image_df[image_df.isTemplate].filename.apply(lambda x: Path(cfg.paths.images_dir.raw)/x)
    ...
    image_df.to_csv(Path(cfg.paths.csv_dir.processed)/"processed_images.csv", encoding="utf-8", index=False)
